...
struct Ray3* initiate_camera_rays(struct Camera* camera){
    ...
    Vec3 y = scalar_mul((Vec3){0,1,0}, camera->hiegth/(camera->halfres_y*2));

    Vec2 ref_angle = get_spherical_angles(camera->camera_screen_dist);
    Vec3 no_rot_camera_screen_dist = (Vec3){0,0,vec3_mag(camera->camera_screen_dist)};
    //Vec3 no_rot_camera_screen_dist = (Vec3){0,0,3};
    ...
    for (int i = -camera->halfres_x; i < camera->halfres_x; i++){
        ...
        for (int j = -camera->halfres_y; j < camera->halfres_y; j++){
            ...
            Vec3 ray_normal = two_vectors_addorsub(no_rot_camera_screen_dist, relx, SUB);
